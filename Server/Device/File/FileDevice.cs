...
namespace Otm.Server.Device.S7
{
    public class FileDevice : IDevice
    {
        ...
        private void GetDeviceParameter(DeviceConfig dvConfig)
        {
            ...
            {
                Logger.LogError($"FileDevice ({Config.Name})|GetDeviceParameter|Error: {ex}");
                throw;
            }
        }
        private void GetDeviceTags(DeviceConfig dvConfig)
        {
            ...
            {
                Logger.LogError($"FileDevice ({Config.Name})|GetDeviceTags|Error: {ex}");
                throw;
            }
        }
        ...
        public void Start(BackgroundWorker worker)
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    {
                        Logger.LogError($"FileDevice ({Config.Name})|Start|Erro nos diretórios: inputReady:{(inputReady ? "Ok" : "Nok")}.  outputReady:{(outputReady ? "Ok" : "Nok")}");
                        configured = false;
                    }
                ...
                catch (Exception ex)
                {
                    ...
                    Logger.LogError($"FileDevice ({Config.Name})|Start|Update Loop Error {ex}");
                    //client.Disconnect();
                }
            }
        }
        ...
        private void ConfigureConnection()
        {
            if (!configured)
            {
                Logger.LogInformation($"FileDevice ({Config.Name})|ConfigureConnection|Configurando conexão...");
                Logger.LogInformation($"FileDevice ({Config.Name})|ConfigureConnection|Instaurando o Watcher na pasta de input: '{inputPath}'");
                ...
                Logger.LogInformation($"FileDevice ({Config.Name})|ConfigureConnection|Utilizando o filtro: '{inputFileFilter}'");
                //watcher.Filter = "*.txt";
        ...
        {
            Logger.LogInformation($"FileDevice ({Config.Name})|ProcessFile|Inicio. fileName: '{fileName}'. fileContent: '{fileContent}'");
        ...
        private void OnChanged(object sender, FileSystemEventArgs e)
        {
            ...
            Logger.LogInformation($"FileDevice ({Config.Name})|OnChanged|Changed: {e.FullPath}");
        }

        private void OnCreated(object sender, FileSystemEventArgs e)
        {
            Logger.LogInformation($"FileDevice ({Config.Name})|OnCreated|Created: {e.FullPath}");
            
            var filename = Path.GetFileName(e.FullPath);
            var fileContent = GetContent(e.FullPath);
            Logger.LogInformation($"FileDevice ({Config.Name})|OnCreated|Content: {fileContent}");
        ...
        private void OnDeleted(object sender, FileSystemEventArgs e)
        {
            ...
            Logger.LogInformation($"FileDevice ({Config.Name})|OnDeleted|Deleted: {e.FullPath}");
            Logger.LogInformation($"FileDevice ({Config.Name})|OnDeleted|Deleted Content: {fileContent}");
        }

        private void OnRenamed(object sender, RenamedEventArgs e)
        {
            Logger.LogInformation($"FileDevice ({Config.Name})|OnRenamed|Renamed:");
            Logger.LogInformation($"FileDevice ({Config.Name})|OnRenamed|    Old: {e.OldFullPath}");
            Logger.LogInformation($"FileDevice ({Config.Name})|OnRenamed|    New: {e.FullPath}");

            string fileContent = GetContent(e.FullPath);
            Logger.LogInformation($"FileDevice ({Config.Name})|OnRenamed|Content: {fileContent}");
        }
        ...
        private void PrintException(Exception? ex)
        {
            if (ex != null)
            {
                Logger.LogError($"FileDevice ({Config.Name})|PrintException|Message: {ex.Message}");
                Logger.LogError($"FileDevice ({Config.Name})|PrintException|Stacktrace:");
                Logger.LogError($"FileDevice ({Config.Name})|PrintException|{(ex.StackTrace)}");
        ...
        public void SetTagValue(string tagName, object value)
        {
            ...
            Logger.LogDebug($"FileDevice ({Config.Name})|SetTagValue|TagName: '{tagName}'|TagValues: '{value}'");
